    ...
    <div class="container">
        <div class="row">
            <div class="col-md-6 col-md-offset-3">
                ...
                <span style="font-size: 24px;"><i class="fa fa-chevron-right"></i> Edit reply</span>
                <!-- NAVIGATION -->
                <br/> <br/>
                @if($errors->any())
                    ...
                    @endforeach
                @endif
                {!! Form::open(['method'=>'post', 'url' => url('/forums/'.$forum->slug.'/topics/'.$topic->id.'/posts/update/'.$post->id)]) !!}
                <label>Reply title:</label>
                <input name="title" type="text" class="form-control" value="@if(old('title')){{old('title')}}@else{{$post->title}}@endif"/> <br/>
                <label>Peply text:</label>
                <textarea name="text" class="form-control" rows="10">@if(old('text')){{old('text')}}@else{{$post->text}}@endif</textarea> <br/>
                <button class="btn btn-orange form-control" type="submit" style="color: black;">
                    <i class="fa fa-reply"></i> Save editing
                </button> <br/> <br/>
